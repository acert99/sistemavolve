...
export async function POST(request: NextRequest) {
  ...
  const vencimentoDate = parseDateOnlyInAppTimeZone(vencimento)
  if (!vencimentoDate) {
    return NextResponse.json(
      { success: false, error: 'Data de vencimento inválida' },
      { status: 400 },
    )
  }
  ...
  try {
    ...
    const dueDateStr = toDateOnlyIsoInAppTimeZone(vencimentoDate)
