...
export async function POST(request: NextRequest) {
  // O Asaas envia o token de webhook no header access_token
  const tokenHeader = request.headers.get('access_token') ?? ''
  ...
  try {
    ...
    await prisma.cobranca.update({
      ...
      data: {
        status: novoStatus,
        ...(isPago ? { pagoEm: payment.paymentDate ? new Date(payment.paymentDate) : new Date() } : {}),
